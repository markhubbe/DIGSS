...
SurveySim<-function(SurveyParameters, artifact.analysis=TRUE, plot=TRUE, plot.artifacts=FALSE,areaprecision=1000){
  ...
  for (a in 1:simulations){
    ...
    for (b in 1:nrow(site.frame)){
      ...
      #surveysHitCoords.x<-NULL
      #surveysHitCoords.y<-NULL
      ...
      for (c in 1:length(yseq)){
        ...
        #if(sum(tmpHit>0)){
        #  for(d in 1:sum(tmpHit)){
        #    surveysHitCoords.x<-c(surveysHitCoords.x,xvals[which(tmpHit==TRUE)[d]])
        #    surveysHitCoords.y<-c(surveysHitCoords.y,yseq[c])
        #  }
        #}
