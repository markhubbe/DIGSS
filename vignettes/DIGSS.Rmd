...
To create parameters for the first time (especially if you are not familiar with R), call `my_parameters<-parametersCreator()` . This will start a text input interface which will allow you to create and/or edit a list of parameters that will be saved in a new list called `my_parameters`. The screenshot below shows what the initial interface looks like.
